		 ...
		 */
		 function generate_zap_button ($order) {
				...
				try {
					...
					/*$invoiceResponse = wp_remote_post ($this->endpoint. '/v1/invoices',
						array(
							'method' => 'POST',
							'timeout' => 45,
							'redirection' => 5,
					...
					$chs = "150x150";
					$choe = "UTF-8";
					$qrcode = 'https://chart.googleapis.com/chart?cht=qr' . '&chs=' . $chs . '&chl=' . $invoiceResponse['payment_request'] . '&choe=' . $choe;
				...
				jQuery(function($){
					...
					function pollingReq () {
						$.get( '" . get_site_url() . "/?wc-api=wc_gateway_zap&ln_invoice=" . $invoiceResponse['payment_request'] . "&order_cb=" . $order->id ."', function( data ) {
							if(data !== 'success'){
								console.log('fodeu');
								window.setTimeout( pollingReq, 2000);
								return;
							}
        ...
        try {
          ...
          if($order->status !== 'completed') {
            ...
            if($order->status == 'processing'){
							...
							if(get_post_meta( $orderId, 'LN_INVOICE', true ) === $payReq) {
								require_once('utilities.php');
								/**
								 * 
								 * Lookup Invoice
